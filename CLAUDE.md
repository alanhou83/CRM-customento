...
2. **【下一个】采购跟单模块**（procurement）：供应商/采购单/到货确认
3. **【之后】库存管理模块**（inventory）：库存台账/收发明细/盘库对账
4. **【之后】财务模块**（finance）：账户流水/应收应付/P&L报表
5. **【之后】报价单生成**：从产品库定制品BOM生成报价单
6. 数据看板优化（待讨论）
7. **【待设计】邮件集成**：CRM内直接发邮件给客户，发送记录自动同步跟进记录
8. Supabase 迁移（后期）
9. 钉钉 H5 嵌入（后期）
